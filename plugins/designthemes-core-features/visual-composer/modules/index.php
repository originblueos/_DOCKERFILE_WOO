...
define( 'DT_VC_CATEGORY', 'Classy Missy' );
...
// Custom Modules
	...
	if ( class_exists( 'WooCommerce' ) ) :
		...
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/recent_products.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/featured_products.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/product.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/products.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/sale_products.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/best_selling_products.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/product_attribute.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/product_category.php';
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/top_rated_products.php';
		
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/single_product.php';
		
		require_once plugin_dir_path( __FILE__ ).'/woocommerce/product_search.php';
	endif;	
...
$attributes = array(
	'type' => 'dropdown',
	'heading' => esc_html__( 'Row stretch', 'classymissy-core' ),
	'param_name' => 'full_width',
	'value' => array(
		esc_html__( 'Default', 'classymissy-core' ) => '',
		esc_html__( 'Default (no paddings)', 'classymissy-core' ) => 'default_no_spaces',
		esc_html__( 'Stretch row', 'classymissy-core' ) => 'stretch_row',
		esc_html__( 'Stretch row and content', 'classymissy-core' ) => 'stretch_row_content',
		esc_html__( 'Stretch row and content (no paddings)', 'classymissy-core' ) => 'stretch_row_content_no_spaces',
	),
	'description' => esc_html__( 'Select stretching options for row and content (Note: stretched may not work properly if parent container has "overflow: hidden" CSS property).', 'classymissy-core' ),
	'weight' => 1
);
...
$attributes = array(
    ...
    'description' => esc_html__( 'Enter inline styles for this row.', 'classymissy-core' )
);
vc_add_param( 'vc_row', $attributes );
...
$attributes = array(
    ...
    'description' => esc_html__( 'Enter text color for this row.', 'classymissy-core' )
);
vc_add_param( 'vc_row', $attributes );
...
$attributes = array(
	'type' => 'dropdown',
	'heading' => esc_html__( 'Spacing', 'classymissy-core' ),
	'param_name' => 'spacing',
	'value' => array(
		esc_html__( 'Default', 'classymissy-core' ) => '',
		esc_html__( 'Default (no paddings)', 'classymissy-core' ) => 'default_no_spaces'
	),
	'description' => esc_html__( 'Default (no paddings) option removes padding of columns inside it.', 'classymissy-core' ),
	'weight' => 1
);
...
$attributes = array(
	'type' => 'el_id',
	'heading' => esc_html__( 'Column ID', 'classymissy-core' ),
	'param_name' => 'el_id',
	'description' => sprintf( esc_html__( 'Enter column ID (Note: make sure it is unique and valid according to <a href="%s" target="_blank">w3c specification</a>).', 'classymissy-core' ), 'http://www.w3schools.com/tags/att_global_id.asp' ),
	'weight' => 1
);
...
$attributes = array(
	'type' => 'dropdown',
	'heading' => esc_html__( 'Content Alignment', 'classymissy-core' ),
	'param_name' => 'el_align',
	'value' => array( esc_html__('None', 'classymissy-core') => '', 'Left' => 'left', 'Right' => 'right', 'Center' => 'center', 'Justify' => 'justify' ),
	'description' => esc_html__( 'Select column alignment.', 'classymissy-core' ),
	'std' => '',
	'weight' => 1
);
vc_add_param( 'vc_column', $attributes );
...
$attributes = array(
    ...
    'heading' => "Inline styles",
    'param_name' => 'addstyles',
    'description' => esc_html__( 'Enter inline styles for this column.', 'classymissy-core' )
);
vc_add_param( 'vc_column', $attributes );
vc_add_param( 'vc_column_inner', $attributes );
...
$attributes = array(
    ...
    'description' => esc_html__( 'Select type of animation for element to be animated when it "enters" the browsers viewport (Note: works only in modern browsers).', 'classymissy-core' ),
	'weight' => 1
);
vc_add_param( 'vc_column_inner', $attributes );

// VC Column Text ----------------------------------------------------------------------
$attributes['description'] = esc_html__( 'Enter inline styles for this textblock.', 'classymissy-core' );
