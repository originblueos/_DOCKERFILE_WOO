<?php 
add_action( 'vc_before_init', 'dt_sc_product_attribute_vc_map' );
function dt_sc_product_attribute_vc_map() {
	...
	vc_map( array( 
		'name' => __( 'Product Attributes', 'js_composer' ),
		'base' => 'dt_sc_product_attribute',
...
add_filter( 'vc_form_fields_render_field_dt_sc_product_attribute_filter_param', 'productAttributeFilterParamValue', 10, 4 );
function productAttributeFilterParamValue( $param_settings, $current_value, $map_settings, $atts ) {

	if ( isset( $atts['attribute'] ) ) {
		$value = getAttributeTerms( $atts['attribute'] );
...
function getAttributeTerms( $attribute ) {
...
# Shortcode
add_shortcode( 'dt_sc_product_attribute', 'dt_sc_product_attribute' );
function dt_sc_product_attribute( $attrs, $content = null ) {
	...
	$query_args = array(
		...
		'tax_query'           => array(
			array(
				'taxonomy' => strstr( $attrs['attribute'], 'pa_' ) ? sanitize_title( $attrs['attribute'] ) : 'pa_' . sanitize_title( $attrs['attribute'] ),
				'terms'    => array_map( 'sanitize_title', explode( ',', $attrs['filter'] ) ),
				'field'    => 'slug'
			)
		)
	);
