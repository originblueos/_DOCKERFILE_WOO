...
if (! class_exists ( 'DTCorePlugin' )) {
	...
	class DTCorePlugin {
		...
		function dtLoadPluginTextDomain() {
			load_plugin_textdomain ( 'classymissy-core', false, dirname ( plugin_basename ( __FILE__ ) ) . '/languages/' );
		}

		/**
		 */
		public static function dtCorePluginActivate() {
			if( ! function_exists('classymissy_option') ){
				wp_die( esc_html__( 'Please make sure "Classy Missy Theme" is activated.', 'classymissy-core' ) );
			}
		}
