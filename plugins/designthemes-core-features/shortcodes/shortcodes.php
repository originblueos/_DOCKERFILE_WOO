...
class DTCoreShortcodesDefination {
	...
	function __construct() {
		...
		add_shortcode("dt_sc_single_product_button", array(
			$this,
			"dt_sc_single_product_button"
		) );

		// Single Product 'Product Tabs'
		add_shortcode("dt_sc_single_product_tabs", array(
			$this,
			"dt_sc_single_product_tabs"
		) );

		// Single Product - Reviews
		add_shortcode("dt_sc_single_product_reviews", array(
			$this,
			"dt_sc_single_product_reviews"
		) );

		// Single Product - Info
		add_shortcode("dt_sc_single_product_info", array(
			$this,
			"dt_sc_single_product_info"
		) );

		// Single Product - Images
		add_shortcode("dt_sc_single_product_slider", array(
			$this,
			"dt_sc_single_product_slider"
		) );

		// Single Product - Show upsell / related prodcuts
		add_shortcode("dt_sc_single_product_upsell", array(
			$this,
			"dt_sc_single_product_upsell"
		) );		
		
		add_shortcode("dt_sc_single_product_price", array(
			$this,
			"dt_sc_single_product_price"
		) );
		// Single Product
	...
	function dt_sc_simple_heading( $attrs, $content = null ){
		...
		if( $type == 'simple' ) {
			...
			$out  = "<div class='dt-sc-title split-title {$class}'>";
			$out .= "<div class='split-title-content'><{$tag}>{$text}</{$tag}></div>";
			$out .= "</div>";
		} elseif( $type == "mz-stripe" ) {			
			$out  = "<div class='dt-sc-title split2-title {$class}'>";
			$out .= "<div class='split2-title-content'><{$tag}>{$text}</{$tag}></div>";
			$out .= "</div>";						
		} elseif( $type == 'ribbon') {
			...
			$out .= "</div>";
		} elseif( $type == 'large-with-subtitle-inside' ){
			$out = "<div class='dt-sc-title large-with-sub-title-inside {$class}'>";
			$out .= "<{$tag}>{$text}</{$tag}>";
			$out .= "<span>{$subtext}</span>";
			$out .= "</div>";			
		} elseif( $type == 'script' ) {
	...
	function dt_sc_search_form($attrs, $content = null) {
		...
		$out .= !empty( $class ) ? '<div class="'.$class.'">' : '';
	...
	function dt_sc_woo_cart($attrs, $content = null) {
		...
		if( function_exists("is_woocommerce")) :
		    $out .= !empty( $class ) ? '<div class="'.$class.'">' : '';
		    $out .= '<a class="cart-info" href="'.WC()->cart->get_cart_url().'" title="'.esc_attr__( 'View Shopping Cart', 'classymissy-core' ).'">';
			...
			$out .= '<p>'.esc_html__('Kindly install & activate woocommerce plugin and check it.', 'classymissy-core').'</p>';
		endif;
	...
	function dt_sc_contact_info($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-contact-info {$type} {$class}'>";
			...
			if( $type == 'type5') {
				...
				if( !empty($icon) ) {
					$out .= "<a class='dt-sc-contact-icon' title='{$a_title}' href='{$a_href}' target='{$a_target}'><span class='icon-wrapper'><span class='{$icon}'> </span></span></a>";
				}
		...
		$out .= DTCoreShortcodesDefination::dtShortcodeHelper ( $content );
		$out .= ( $type == 'type8' ) ? '</div> <!-- .contact-details --> ': '';
		$out .= "</div>";
	...
	function dt_sc_mc_newsletter($attrs, $content = null) {
		extract ( shortcode_atts ( array (
			...
			'tooltip' => '',
			'listid' => '',
			'name' => esc_html__('Your Name', 'classymissy-core'),
			'email' => esc_html__('Your Email Address', 'classymissy-core'),
			'button' => esc_html__('Subscribe Now', 'classymissy-core'),
			'show_name' => '',
			'class' => ''
		), $attrs ) );
		...
		$out = "<div class='dt-sc-newsletter-section {$type} {$class}'>";
			...
			$mcapi = classymissy_option('layout','mailchimp-key');
			...
			$out .= '<form class="dt-sc-subscribe-frm" name="frmsubscribe" action="#" method="post">';
				...
				if( $type == 'type6' ) {
					$out .= '<span class="input-border-style"></span>';
                }
	...
	function dt_sc_team($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-team {$teamstyle} {$class}'>";
			...
			if( $teamstyle == 'simple' ) {
				...
				$out .= !empty( $content ) ? '<div class="dt-sc-team-desc">'. $content .'</div>' : '';
	...
	function dt_sc_tm_carousel_item($attrs, $content = null) {
		...
		$out = '<li>';
			$out .= "<div class='dt-sc-testimonial {$type} {$class}'>";
				...
				if($type == 'type1' || $type =='type3' || $type == 'type6')
					$out .= $img.$con;
				else
					$out .= $con.$img;
	...
	function dt_sc_twitter_tweets($attrs, $content = null) {
		...
		if($username && $consumerkey && $consumersecret && $accesstoken && $accesstokensecret) {
			...
			require_once(CLASSYMISSY_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php");
			...
			$out .= '<div class="dt-sc-twitter-feeds">';
				...
				if($twitter && is_array($twitter)) {
					...
					$out .= '<div class="no-tweets">'.esc_html__('No Twitter Tweets found.','classymissy-core').'</div>';
				}
			$out .= '</div>';
		} else {
			$out .= '<div class="no-tweets">'.esc_html__('Invalid Twitter Authentication keys.','classymissy-core').'</div>';
		}
			...
			while( $the_query->have_posts() ):
				...
				$out .= '<article id="post-'.esc_attr($id).'" class="'.esc_attr($post_classes).'">';
					...
					if( $style == "entry-date-left") :
						...
						$out .= '	</div>';
									...
									if( strtolower($allow_excerpt) == 'yes' ):
										$out .= '<div class="entry-body">'.classymissy_excerpt($excerpt_length).'</div>';
									endif;
					...
					elseif( $style == "entry-date-author-left"):
						...
						$out .= 			esc_html__('Posted on','classymissy-core').get_the_date(' d M Y');
						$out .= '		</div>';
						$out .= '		<div class="comments '.esc_attr($hide_comment_meta).'">';
											$commenttext = "";
											if((wp_count_comments($id)->approved) == 0):
											...
											$out .= '/ <a class="comments" href="'.esc_url($link).'/#respond">';
											$out .= '	<i class="pe-icon pe-chat"></i> '.esc_html($commenttext).' '.esc_html__('Comments','classymissy-core');
											$out .= '</a>';
						$out .= '		</div>';
	...
	function dt_sc_recent_post( $attrs, $content = null ){
		...
		$warning = esc_html__('No Posts Found','classymissy-core');
		
		if( !empty($category) ){
			$args = array( 'posts_per_page' => $count, 'orderby' => 'date', 'cat' => $category );
			$warning = esc_html__('No Posts Found in Category ','classymissy-core').$category;
		}
		...
		if($the_query->have_posts()): $i = 1;
		 while($the_query->have_posts()): $the_query->the_post();
			...
			$out .= "<div class='blog-entry blog-thumb-style entry-date-left {$lastcls}'>";
				$out .= '<div class="entry-thumb">';
					$out .= get_the_post_thumbnail( get_the_ID(), 'classymissy-blog-thumb' );
				$out .= '</div>';
				$out .= '<div class="entry-details">';
	...
	function dt_sc_portfolios( $attrs, $content = null ){
		...
		if( (sizeof($categories) > 1) && ( $allow_filter == "yes" ) ) :
			...
			$out .= '	<a href="#" class="active-sort" title="" data-filter=".all-sort">'.esc_html__('All','classymissy-core').'</a>';
		...
		if($the_query->have_posts()) :
			...
			if( $ajax_call == 'no' ) :
				...
				if( $display_style == 'load-more' || $display_style == 'lazy' ) :
					$label = ( $display_style == 'load-more' ) ? esc_html__('Load More', 'classymissy-core' ) : esc_html__('Scroll To Load More','classymissy-core');
	...
	function dt_sc_events_list( $attrs, $content = null ){
		...
		$out = '<div class="dt-sc-events-list-wrapper">';
			...
			if($events):							
				...
				$out .= '<div class="dt-sc-warning-box">'.esc_html__('No Events Found','classymissy-core').'</div>';
			endif;	
			wp_reset_postdata();
							...
							$out .= '<div class="dt-sc-event type1">';
								 ...
								 $out .= '<div class="dt-sc-event-thumb">';
									if(has_post_thumbnail()):
										...
										$out .= get_the_post_thumbnail($post->ID, 'classymissy-event-list2', $attr);
									else:
							...
							$out .= '<div class="dt-sc-event type5">';
								...
								$out .= '<div class="dt-sc-one-half column">';
								   $out .= '<div class="dt-sc-event-meta">';
										...
										$out .= '<a href="'.get_permalink().'" class="dt-sc-events-read-more">'.esc_html__('Find out more', 'classymissy-core').'</a>';
								   $out .= '</div>';
								$out .= '</div>';
							$out .= '</div>';
						$out .= '</div>';
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		if( !classymissy_option('pageoptions','show-launchdate') ) return;

		$out .= '<div class="downcount '.$class.'" data-date="'.classymissy_opts_get('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.classymissy_opts_get('comingsoon-timezone', '0').'">';
			$out .= '<div class="dt-sc-counter-wrapper">';
				$out .= '<div class="counter-icon-wrapper">';
					$out .= '<div class="dt-sc-counter-number days">00</div>';
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Days Left', 'classymissy-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Days', 'classymissy-core').'</h3>';
			$out .= '</div>';
			$out .= '<div class="dt-sc-counter-wrapper">';
				$out .= '<div class="counter-icon-wrapper">';
				...
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Hr.', 'classymissy-core').'</h3>';
				elseif($type == 'type5')
					$out .= '<h3 class="title">'.esc_html__('Hrs', 'classymissy-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Hours', 'classymissy-core').'</h3>';
			$out .= '</div>';
			$out .= '<div class="dt-sc-counter-wrapper">';
				$out .= '<div class="counter-icon-wrapper">';
				...
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Min.', 'classymissy-core').'</h3>';
				elseif($type == 'type5')
					$out .= '<h3 class="title">'.esc_html__('Mins', 'classymissy-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Minutes', 'classymissy-core').'</h3>';
			$out .= '</div>';
			$out .= '<div class="dt-sc-counter-wrapper last">';
				...
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Sec.', 'classymissy-core').'</h3>';
				elseif($type == 'type5')
					$out .= '<h3 class="title">'.esc_html__('Secs', 'classymissy-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Seconds', 'classymissy-core').'</h3>';
			$out .= '</div>';
		$out .= '</div>';
	...
	function dt_sc_sociable($attrs, $content = null) {
		...
		$socials = classymissy_option('social');
		$out = "";

		if(!empty($socials)):
			$out .= "<ul class='dt-sc-sociable {$style} {$class}'>";
	 ...
	 */
	function dt_sc_single_product_button( $attrs, $content = null ) {
		...
		$output .= "<div class='single-woo-purchase-button-container'>";
		ob_start();
		wc_clear_notices();
	 ...
	 * Single product : Reviews
	 */
	function dt_sc_single_product_reviews( $attrs, $content = null ) {

		global $woocommerce, $product;
		if(!is_object($woocommerce) || !is_object($woocommerce->query) || empty($product)) return;

		$output = "<div class='single-product-reviews-container'>";
		ob_start();
	 ...
	 * Single product : Info
	 */
	function dt_sc_single_product_info( $attrs, $content = null ) {

		global $woocommerce, $product;
		if(!is_object($woocommerce) || !is_object($woocommerce->query) || empty($product)) return;

		$output = "<div class='single-product-infos-container'>";
		ob_start();
	...
	/**
	 * Single Product : Up sell / Related Product
	 */
	function dt_sc_single_product_upsell( $attrs, $content = null ) {
		extract ( shortcode_atts ( array (
			'show' => 'upsells related',
			'count' => '3'
		), $attrs ) );
		...
		if(strpos($show, 'upsells') !== false) {
			ob_start();
			$output .= "<div class='single-woo-product-upsells-container{$style}'>";
			woocommerce_upsell_display($count, $count);
			$output .= ob_get_clean();
			$output .= "</div>";
		}

		if(strpos($show, 'related') !== false) {
			ob_start();
			$output .= "<div class='single-woo-related-product-container{$style}'>";
			woocommerce_related_products( array('posts_per_page'=>$count, 'columns'=>$count) );
			$output .= ob_get_clean();
			$output .= "</div>";			
		}
	 ...
	 */
	function dt_sc_single_product_price( $attrs, $content = null ) {
		global $woocommerce, $product;
		
		if(!is_object($woocommerce) || !is_object($woocommerce->query) || empty($product)) return;
		
		if ( $price_html = $product->get_price_html() ) {
			return '<div class="single-product-price">'.$price_html.'</div>';
		} else {
			return;
		}
	}
	...
	function dt_sc_google_map( $attrs, $content = null ) {
		...
		$api_key = classymissy_option('layout','google-map-key');
