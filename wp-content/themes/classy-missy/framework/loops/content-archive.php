<?php
	$page_layout = classymissy_option('pageoptions','post-archives-page-layout');
	...
	$post_layout = classymissy_option('pageoptions','post-archives-post-layout');
	...
	$excerpt = classymissy_option('pageoptions','post-archives-excerpt');
	...
	$show_post_format = classymissy_option('pageoptions','post-format-meta'); 
	$show_post_format = isset( $show_post_format )? "" : "hidden";
	
	$show_author_meta = classymissy_option('pageoptions','post-author-meta');
	$show_author_meta = isset( $show_author_meta ) ? "" : "hidden";
	
	$show_date_meta = classymissy_option('pageoptions','post-date-meta');
	$show_date_meta = isset( $show_date_meta ) ? "" : "hidden";	

	$show_comment_meta = classymissy_option('pageoptions','post-comment-meta');
	$show_comment_meta = isset( $show_comment_meta ) ? "" : "hidden";

	$show_category_meta = classymissy_option('pageoptions','post-category-meta');
	$show_category_meta = isset( $show_category_meta ) ? "" : "hidden";
	
	$show_tag_meta = classymissy_option('pageoptions','post-tag-meta');
	$show_tag_meta = isset( $show_tag_meta ) ? "" : "hidden";

	$post_style = classymissy_option('pageoptions','post-style');
	...
	if( have_posts() ) :
                        ...
                        <?php if( $format == "image" || empty($format) ) :
                                if( has_post_thumbnail() ) :?>
                                    <div class="entry-thumb">
                                        <a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s','classy-missy'),the_title_attribute('echo=0'));?>"><?php the_post_thumbnail("full");?></a>
                        ...
                        <?php if( $post_style == "entry-date-left"): ?>
                                ...
                                <div class="entry-details">
                                	...
                                	<div class="entry-title">
                                    	<h4><a href="<?php the_permalink();?>" title="<?php printf( esc_attr__('Permalink to %s','classy-missy'), the_title_attribute('echo=0'));?>"><?php the_title(); ?></a></h4>
                                    </div>
                                    
                                   <?php if( isset($allow_excerpt) && isset($excerpt) ):?>
                                    		<div class="entry-body"><?php echo classymissy_excerpt($excerpt);?></div>
                                   <?php endif;?>
                                   ...
                                   <div class="entry-meta-data">
                                   		...
                                   		<p class="author <?php echo esc_attr( $show_author_meta );?>">
                                            ...
                                            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                            	title="<?php esc_attr_e('View all posts by ', 'classy-missy'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                                        </p>
                                   ...
                                   <?php if( isset($allow_read_more) && isset($read_more) ):
								   			if( shortcode_exists( 'dt_sc_button' ) && classymissy_is_plugin_active('js_composer/js_composer.php') ):
												$read_more = stripcslashes($read_more);
												$sc = str_replace("]",' link="url:'.$link.'"]',$read_more);
												$sc = do_shortcode($sc);
												echo !empty( $sc ) ? $sc : '';
											else:
												echo '<a href="'.get_permalink().'" title="'.get_the_title().'" class="dt-sc-button medium icon-right with-icon rounded-border read-more">'.esc_html__('Read More', 'classy-missy').'<span class="fa fa-long-arrow-right"> </span></a>';
											endif;
                        ...
                        <?php elseif( $post_style == "entry-date-author-left"):?>
                        		...
                        		<div class="entry-date-author">
                                    ...
                                    <div class="entry-author <?php echo esc_attr( $show_author_meta );?>">
                                    	...
                                    	<a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                        	title="<?php esc_attr_e('View all posts by ', 'classy-missy'); echo get_the_author();?>"><span><?php echo get_the_author();?></span></a>
                                    </div>
                        ...
                        <?php else: # Default Post Style ?>
                        		<div class="entry-details">
                                    ...
                                    <div class="entry-meta">
                                    	...
                                    	<div class="date <?php echo esc_attr($show_date_meta);?>"><?php 
											esc_html_e('Posted on ','classy-missy');
											echo get_the_date ( get_option('date_format') );?></div>
        ...
        <!-- **Pagination** -->
        <div class="pagination blog-pagination"><?php echo classymissy_pagination();?></div><!-- **Pagination** -->
        <!-- Blog Template Ends --><?php

	else:?>
    	<h2><?php esc_html_e('Nothing Found.', 'classy-missy'); ?></h2>
        <p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'classy-missy'); ?></p><?php
	endif;?>