<?php 
class ClassyMissy_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_flickr', 'description'=>esc_html__('A widget that show last flickr photo streams', 'classy-missy'));
		parent::__construct(false,THEME_NAME.esc_html__(' Flickr Widget','classy-missy'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo ($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID:','classy-missy');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','classy-missy');?>
         ...
         <p><label for="<?php echo ($this->get_field_id('show'));?>"><?php esc_html_e('What pictures to display','classy-missy'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('show')); ?>" name="<?php echo ($this->get_field_name('show')); ?>">
            <?php  $a = array("latest"=>esc_html__("Latest",'classy-missy'),"random"=>esc_html__("Random",'classy-missy'));
         ...
         <p><label for="<?php echo ($this->get_field_id('size'));?>"><?php esc_html_e('What pictures to display','classy-missy'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('size')); ?>" name="<?php echo ($this->get_field_name('size')); ?>">
            <?php  $a = array("s"=>esc_html__("square",'classy-missy'),"t"=>esc_html__("thumbnail",'classy-missy'), "m" => esc_html__("medium",'classy-missy'));
			foreach($a as $key => $value ):
