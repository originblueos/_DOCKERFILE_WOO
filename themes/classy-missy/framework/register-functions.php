 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('classymissy_features')) {

	function classymissy_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'classymissy-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'classymissy-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'classymissy-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'classymissy-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'classymissy-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'classymissy-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list

			if(class_exists('DTRestaurantAddon')):
				add_image_size( 'classymissy-menu-type2', 130, 130, true  ); // menu type2
				add_image_size( 'classymissy-chef-type1', 420, 383, true  ); // chef type1
				add_image_size( 'classymissy-chef-type2', 420, 420, true  ); // chef type2
			endif;

			if(class_exists('DTRoomAddon'))
				add_image_size( 'classymissy-room-type', 400, 297, true  ); // room type
				
			if(class_exists('DTEventAddon')):
				add_image_size( 'classymissy-events-grid', 375, 375, true  ); // events grid
				add_image_size( 'classymissy-events-list', 420, 285, true  ); // events list
			endif;
			
			if(class_exists('DTProgramAddon')):
				add_image_size( 'classymissy-training-list2', 280, 311, true  ); // training list2
			endif;
		}
	...
	add_action('after_setup_theme', 'classymissy_features');
}

/* ---------------------------------------------------------------------------
 * When theme activation, update default option
 * --------------------------------------------------------------------------- */
if (!function_exists('classymissy_activation_function')) {

	// check activate theme
	function classymissy_activation_function($oldname, $oldtheme=false) {
		if(!in_array($oldname, array('Classy Missy', 'Classy Missy Child'))) {
			update_option(CLASSYMISSY_SETTINGS, classymissy_default_option());
		}
	}
	add_action("after_switch_theme", "classymissy_activation_function", 10 , 2);
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('classymissy_under_construction') ){
	function classymissy_under_construction(){
...
if( classymissy_option('pageoptions','enable-comingsoon') ):
	add_action('template_redirect', 'classymissy_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'classymissy_rand_css', 101);
	function classymissy_rand_css() {
		$id = classymissy_option('pageoptions','comingsoon-pageid');
		if ( $id ) {
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('classymissy_wp_list_categories') ){
	function classymissy_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'classymissy_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('classymissy_wp_list_archive') ){
	function classymissy_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'classymissy_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('classymissy_wpcf7_form_elements') ){
	function classymissy_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'classymissy_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Get Current Page or Page ID
 * --------------------------------------------------------------------------- */
function classymissy_ID(){
	global $post;
	$postID = false;
 ...
 * --------------------------------------------------------------------------- */
function classymissy_slider( $id = false ){
	...
	if( $id || is_home() || get_post_type() == 'page' ) {
		...
		elseif( is_page() ) $id = classymissy_ID();
		...
		if( is_array( $slider_key) && array_key_exists('slider_type', $slider_key) ) {
			...
			if( ( $slider_key['slider_type'] == 'revolutionslider' ) && ( array_key_exists( 'revolutionslider_id', $slider_key ) ) && ( array_key_exists( 'show_slider', $slider_key ) ) ) {
				...
				$slider = '<div id="slider"><div class="dt-sc-main-slider" id="dt-sc-custom-slider">';
					$slider .= classymissy_wp_kses(do_shortcode( $slider_key['customslider_sc'] ));
				$slider .= '</div></div>';
			}
		}
	}
 ...
 * --------------------------------------------------------------------------- */
function classymissy_pagination( $query = false, $load_more = false ){
	global $wp_query;
	...
	$end_size = 1;
	$mid_size = 2;
	$show_all = classymissy_option('general','showall-pagination');
	$dots = false;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'classy-missy') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'classy-missy'),
					'next_text'          => esc_html__('Next', 'classy-missy').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
 ...
 * Page Title
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'classymissy_page_title' ) ){
	function classymissy_page_title( $echo = false ){
		...
		if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ){
			...
			$title = get_the_title( classymissy_ID() );
 ...
 * Breadcrumbs
 * --------------------------------------------------------------------------- */
function classymissy_breadcrumbs(){
	global $post;
	
	$homeLink = esc_url( home_url('/') );
	$separator = '<span class="'.classymissy_option('layout','breadcrumb-delimiter').'"></span>';
	...
	if( function_exists('is_bbpress') && is_bbpress() ){
		bbp_breadcrumb( array(
			...
			'after' 		=> '</div>',
			'sep' 			=> '<span class="'.classymissy_option('layout','breadcrumb-delimiter').'"></span>',
			'crumb_before' 	=> '',
			'crumb_after' 	=> '',
			'home_text' 	=> esc_html__('Home','classy-missy'),
		) );
		return true;
	}
	...
	$breadcrumbs[] =  '<a href="'. $homeLink .'">'. esc_html__('Home','classy-missy') .'</a>';
	...
	if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ) {
		...
		$breadcrumbs[] = '<a href="">'.esc_html__('Search', 'classy-missy').'</a>';

	} elseif( is_single() && ! is_attachment() ){

		if( get_post_type() == 'dt_portfolios' ){

			$cat = get_the_term_list(classymissy_ID(), 'portfolio_entries', '', '$$$', '');
			...
			$cat = get_the_term_list(classymissy_ID(), 'product_cat', '', '$$$', '');
			$cats = array_filter(explode('$$$', $cat));
			if (!empty($cats))
				$breadcrumbs[] = $cats[0];
		...
		$breadcrumbs[] = '<span class="current">'. get_the_title( classymissy_ID() ) .'</span>';
 ...
 * Breadcrumb Section
 * --------------------------------------------------------------------------- */
function classymissy_breadcrumb_section($id, $post_type ) {

	$bcrumb = classymissy_option('layout','show-breadcrumb');
	if( isset($bcrumb) && $bcrumb == 'true' ) :
		...
		if(!empty($bg) || !empty($co)) :
			...
			$color = !empty($settings['sub-title-bg-color']) ? classymissy_hex2rgb($settings['sub-title-bg-color']) : '';
		else:
			$bg = classymissy_option('layout','sub-title-bg');
			$opacity = classymissy_option('layout','sub-title-opacity');
			$opacity = !empty($opacity) ? $opacity : '1';
			$position = classymissy_option('layout','sub-title-bg-position');
			$position = !empty($position) ? $position : 'center center';
			$repeat = classymissy_option('layout','sub-title-bg-repeat');
			$repeat = !empty($repeat) ? $repeat : 'repeat';
			$color = classymissy_option('layout','sub-title-bg-color');
			$color = !empty($color) ? classymissy_hex2rgb($color) : '';
		endif;

		$style = !empty($bg) ? "background:url($bg) $position $repeat;" : "";
		$style .= !empty($color) ? "background-color:rgba(  $color[0] ,  $color[1],  $color[2], {$opacity});" : "";

		$bstyle = classymissy_option('layout','breadcrumb-style');
		$bstyle .= classymissy_option('layout','breadcrumb-darkbg') ? ' dt-sc-dark-bg ' : '';
...
function classymissy_breadcrumb_section_with_class ( $title , $class ){
	$bcrumb = classymissy_option('layout','show-breadcrumb');
	if( isset($bcrumb) && $bcrumb == 'true' ) :
...
function classymissy_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'classy-missy' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'classy-missy'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'classy-missy' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'classy-missy' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'classy-missy' );
		}
		...
		$title = esc_html__('Tag Archives','classy-missy');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function classymissy_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function classymissy_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('classymissy_hex2rgb')) {

	function classymissy_hex2rgb($hex) {
		$hex = str_replace ( "#", "", $hex );
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'classymissy_opts_get' ) ) {

	function classymissy_opts_get( $opt_name, $default = null ){

		// getting option
		$opts = get_option ( CLASSYMISSY_SETTINGS );
		$opts = is_array( $opts ) ? $opts : array();

		$value = classymissy_array_search($opts, $opt_name);
		if( ! empty( $value ) ){
			return $value;
		} else {
			return $default;
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'classymissy_opts_show' ) ) {

	function classymissy_opts_show( $opt_name, $default = null ){
 ...
 * --------------------------------------------------------------------------- */
function classymissy_array_search(array $array, $search) {
	$keys = false;
	...
	foreach ($array as $key => $value) {
		if (is_array($array[$key])) {
			if (array_key_exists($search, $array[$key])) {
			    ...
			    $keys = classymissy_array_search($array[$key], $search);
			}
		}
	}
	return $keys;
 ...
 * Theme Header Logo
 * --------------------------------------------------------------------------- */
function classymissy_header_logo() {
	echo '<div id="logo">';
		if( classymissy_option('layout', 'logo') ):
			$url = classymissy_option('layout', 'logo-url');
			$url = !empty( $url ) ? $url : CLASSYMISSY_THEME_URI . "/images/logo.png";

			$darkbg_url = classymissy_option('layout','darkbg-logo-url');
			$darkbg_url = !empty($darkbg_url) ? $darkbg_url : CLASSYMISSY_THEME_URI ."/images/light-logo.png"; ?>
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function classymissy_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'classy-missy');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'classy-missy'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'classy-missy');
					endif;
					edit_comment_link(esc_html__('Edit', 'classy-missy'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'classy-missy'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function classymissy_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function classymissy_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $classymissy_like_love;
	...
	switch($variable) {
		...
		case 'pagenow':
			return $pagenow;
		break;
		case 'classymissy_like_love':
			return $classymissy_like_love;
		break;
	}
	return false;
}
...
* break oembeds.
*/
function classymissy_undo_recurrence_oembed_logic() {
...
add_action( 'init', 'classymissy_undo_recurrence_oembed_logic' );?>